...
class TradingEnvironment:
    ...
    def step(self, action):
        ...
        ema9_current, ema20_current = self.ema9[self.current_step], self.ema20[self.current_step]
        ema9_prev, ema20_prev = self.ema9[self.current_step - 1], self.ema20[self.current_step - 1]

        if action == 1 and ema9_prev < ema20_prev < ema9_current > ema20_current and price > self.open_price[
            self.current_step]:
            loss_pct = 0.03
            gain_pct = 0.15
            stop_loss = ema20_current
            ...
            for i in low_and_high:
                ...
                if high >= tp_limit:  # Take Profit hit1
                    ...
                    break
        elif action == -1 and ema9_prev > ema20_prev > ema9_current < ema20_current and price < self.open_price[
            self.current_step]:
            loss_pct = 0.03
            gain_pct = 0.15
            stop_loss = ema20_current
...
def test_model(genome, test_data, config_path):
    ...
    while True:
        ...
        balance, done = env.step(action)  # Step the environment with the chosen action
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            if ema9[-3]<ema20[-3] and ema9[-2]>ema20[-2] and open_symbolused[-2]<close_symbolused[-2]:  # Buy action
            ...
            elif ema9[-3]>ema20[-3] and ema9[-2]<ema20[-2] and open_symbolused[-2]>close_symbolused[-2]:  # Sell action
                ...
                console.print(f'[green]Action: Hold. High: {prev_high}. Low: {prev_low}. Open: {prev_open}. Close: {prev_price}. EMA20: {ema20[-2]}[green/]')
